...
impl Graph {
    // fn parameter edge_list is an array of
    // source, destination and cost
    fn from_edge_list(edge_list: &Vec<(Node, Node, Cost)>) -> Self {
        // list of a node and its adjacent nodes with associated cost
...
fn shortest_path(g: &Graph, start: Node, goal: Node) -> Option<(Vec<Node>, Cost)> {
    ...
    // No paths found between `start` and `goal`
    None
}
